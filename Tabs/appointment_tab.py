...
def _palette() -> dict:
    ...
    try:
        ...
        # THEME values override defaults; missing keys fall back
...
class AppointmentTab(QtWidgets.QWidget):
    ...
    def _tab_qss(self) -> str:
        p = _palette()  # merges UI.design_system.COLORS over safe defaults
        return f"""
        ...
        QPushButton {{
            background:{p.get('primary', '#3A8DFF')};
        ...
        QPushButton[variant="info"]    {{ background:{p.get('info', '#2CBBA6')};    color:white; }}
        QPushButton[variant="success"] {{ background:{p.get('success', '#7A77FF')}; color:white; }}
