...
try:
    from core import app_settings as AS  # read_all(), DEFAULTS
except Exception:
...
class AccountsTab(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._tax_pct: float = 0.0
        self._default_method: str = "Cash"
    ...
    def _build_ui(self):
        ...
        subtitle.setStyleSheet("color:#64748b; margin-left:6px;")
        ...
        ctx_bar = QtWidgets.QFrame(); ctx_bar.setProperty("modernCard", True)
        ctx_layout = QtWidgets.QHBoxLayout(ctx_bar); ctx_layout.setContentsMargins(12, 8, 12, 8); ctx_layout.setSpacing(8)
        self.lbl_ctx = QtWidgets.QLabel("")
        self.lbl_ctx.setStyleSheet("color:#334155;")
        ctx_layout.addWidget(self.lbl_ctx)
        ctx_layout.addStretch(1)
        root.addWidget(ctx_bar)
        ...
        self.lbl_billing_hint = QtWidgets.QLabel("")
        self.lbl_billing_hint.setStyleSheet("color:#64748b;")

        sec_bill = CollapsibleSection(_tr("Billing"), _tr("Auto-calculates owed = total(±tax) − paid"))
        ...
        sec_bill.add_full_row(3, self.lbl_billing_hint)
    ...
    def apply_settings(self, cfg: Dict[str, object]):
        """Apply relevant settings from core/app_settings.py."""
        try:
            self._currency = str(cfg.get("bill/currency", "USD"))
            self._tax_pct = float(cfg.get("bill/tax_pct", 0.0))
            self._default_method = str(cfg.get("bill/default_method", "Cash"))

            rtl = str(cfg.get("lang/rtl", False)).lower() in ("1", "true", "yes")
            self._rtl = rtl
            self.setLayoutDirection(QtCore.Qt.RightToLeft if rtl else QtCore.Qt.LeftToRight)

            # Update context strip
            self.lbl_ctx.setText(
                f"{_tr('Currency')}: {self._currency}   |   "
                f"{_tr('Tax')}: {self._tax_pct:.2f}%   |   "
                f"{_tr('Default Method')}: {self._default_method}"
            )
            # Billing hint
            self.lbl_billing_hint.setText(
                _tr("Owed = (Total × (1 + Tax)) − Paid  •  Currency: ") + self._currency
            )
            # Currency symbol in headers
            self._update_money_headers()

            # Recompute owed
            self._recalc_owed()
            if self.table.rowCount() > 0:
                self._recalc_table_owed()
        except Exception:
            traceback.print_exc()

    def _update_money_headers(self):
        sym = _cur_sym(self._currency)
        self.table.setHorizontalHeaderLabels([
            _tr("Name"),
            _tr("Age"),
    ...
    def set_billing_context(self, ctx: Dict[str, object]):
        """Optional external hook (called from HomePage/apply_to_home)."""
        merged = {
            "bill/currency": ctx.get("currency", self._currency),
            "bill/tax_pct": ctx.get("tax_pct", self._tax_pct),
            "bill/default_method": ctx.get("default_method", self._default_method),
            "lang/rtl": self._rtl,
        }
    ...
    def _recalc_owed(self):
        total = float(self.in_total.value())
        paid = float(self.in_paid.value())
        owed = max(0.0, (total * (1.0 + self._tax_pct / 100.0)) - paid)
        self.in_owed.blockSignals(True)
        self.in_owed.setValue(owed)
        self.in_owed.blockSignals(False)

    def _recalc_table_owed(self):
        self.table.blockSignals(True)
        try:
            for r in range(self.table.rowCount()):
                total_paid = _to_float(self._txt(r, self.COL_PAID))
                total_amount = _to_float(self._txt(r, self.COL_TOTAL))
                owed_val = max(0.0, (total_amount * (1.0 + self._tax_pct / 100.0)) - total_paid)
                self.table.setItem(r, self.COL_OWED, QtWidgets.QTableWidgetItem(f"{owed_val:.2f}"))
                it = self.table.item(r, self.COL_OWED)
                if it:
                    it.setTextAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
                    it.setData(QtCore.Qt.UserRole, owed_val)

                # Gentle highlight if balance owed
                bg = None
                if owed_val > 0.01:
                    bg = QtGui.QColor("#FFF8E1")
                for c in range(self.table.columnCount()):
                    itc = self.table.item(r, c)
                    if itc:
                        itc.setBackground(QtGui.QBrush(bg) if bg else QtGui.QBrush())
        finally:
    ...
    def _add_client_from_form(self):
        ...
        if any((c.get("Name") or "").strip().lower() == name.lower() for c in self.clients):
            if QtWidgets.QMessageBox.question(
                ...
                return
        total_raw = float(self.in_total.value())
        paid_raw = float(self.in_paid.value())
        owed_val = max(0.0, (total_raw * (1.0 + self._tax_pct / 100.0)) - paid_raw)
        ...
        payload = {
            ...
            "_TaxPct": self._tax_pct,
            "_DefaultMethod": self._default_method,
        }
        try:
    ...
    def _on_cell_changed(self, row, column):
        ...
        try:
            ...
            owed = max(0.0, (total_amount * (1.0 + self._tax_pct / 100.0)) - total_paid)
    ...
    def _export_csv(self):
        ...
        headers = [
            _tr("Name"), _tr("Age"),
            f"{_tr('Total Paid')} ({_cur_sym(self._currency)})",
            f"{_tr('Owed')} ({_cur_sym(self._currency)})",
            f"{_tr('Total Amount')} ({_cur_sym(self._currency)})",
            _tr("Has Photo"), _tr("Currency"), _tr("Tax %"), _tr("Default Method")
        ]
        try:
            buf = io.StringIO()
            ...
            for r in range(self.table.rowCount()):
                ...
                row += [self._currency, f"{self._tax_pct:.2f}", self._default_method]
                writer.writerow(row)
        ...
        for row in range(self.table.rowCount()):
            ...
            if it0 and it0.text().strip().lower() == key:
                for col in range(self.table.columnCount()):
                    ...
                    if it: it.setBackground(QtGui.QBrush(QtGui.QColor("#E0F2FE")))
...
if __name__ == "__main__":
    ...
    w.show()
    try:
        from UI import design_system as mt
        # mt.apply_window_backdrop(w, prefer_mica=True)
    except Exception:
        pass
    sys.exit(app.exec_())